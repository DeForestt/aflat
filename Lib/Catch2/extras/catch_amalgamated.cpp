            ...
            [n, first, &estimator, &dist, &rng] {
                ...
                std::generate_n(
                    std::back_inserter( resampled ), n, [first, &dist, &rng] {
                        return first[static_cast<std::ptrdiff_t>(
                            dist( rng ) )];
                    } );
...
namespace Catch {
    ...
    Config::Config( ConfigData const& data ): m_data( data ) {
        ...
        if ( provideBazelReporterOutput() ) {
            ...
            // validation, since the key is hardcoded, this should not be an
            // issue.
#    pragma warning( push )
#    pragma warning( disable : 4996 )
#endif
    ...
    namespace {
        ...
        void printTotals( std::ostream& out,
            ...
            if ( totals.testCases.total() == 0 ) {
                ...
                auto guard = colourImpl->guardColour( Colour::ResultError )
                                 .engage( out );
    ...
    void XmlReporter::testRunStarting( TestRunInfo const& testInfo ) {
        ...
        if ( m_config->testSpec().hasFilters() )
            m_xml.writeAttribute(
                "filters"_sr, serializeFilters( m_config->getTestsOrTags() ) );
    }
