...
#                endif
                static Catch::Detail::unique_ptr<
...
namespace Catch {
    ...
    public:
        ...
        friend auto operator!=( ExprLhs&& lhs, RhsT&& rhs )
            -> BinaryExpr<LhsT, RhsT const&> {
            return {
                compareNotEqual( lhs.m_lhs, rhs ), lhs.m_lhs, "!="_sr, rhs };
        }
        template <typename RhsT,
                  std::enable_if_t<std::is_arithmetic<RhsT>::value, int> = 0>
        friend auto operator!=( ExprLhs&& lhs, RhsT rhs )
...
#                        define CATCH_INTERNAL_DEFINE_EXPRESSION_OPERATOR(     \
                            ...
                            template <                                         \
                                ...
                                std::enable_if_t<                              \
                                    ...
                                    int> = 0>                                  \
                            friend auto operator op( ExprLhs&& lhs,            \
                                                     RhsT&& rhs )              \
                                ->BinaryExpr<LhsT, RhsT const&> {              \
                                return {                                       \
    ...
    namespace Generators {
        ...
        random( T a, T b ) {
            static_assert( !std::is_same<T, char>::value &&
                               ...
                               !std::is_same<T, bool>::value,
                           "The requested type is not supported by the "
                           "underlying random distributions from std" );
            return GeneratorWrapper<T>(
    ...
    namespace Matchers {
        ...
        public:
            ...
            template <typename RangeLike> bool match( RangeLike&& rng ) const {
                ...
                return end( rng ) !=
                       std::find_if(
                           begin( rng ), end( rng ), [&]( auto const& elem ) {
...
namespace Catch {
    ...
    private:
        ...
        void printSummaryRow( StringRef label,
