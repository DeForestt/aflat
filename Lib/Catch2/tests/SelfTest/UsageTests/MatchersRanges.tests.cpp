...
namespace {
    ...
    namespace unrelated {
        template <typename T> class needs_ADL_begin {
        ...
        public:
            ...
            needs_ADL_begin( std::initializer_list<T> init ):
                m_elements( init ) {}
...
#    pragma clang diagnostic ignored "-Wunused-function"
    ...
    template <typename T> class has_different_begin_end_types {
...
#    pragma clang diagnostic pop
    ...
    template <typename T> struct with_mocked_iterator_access {
        ...
        template <bool is_const> class basic_iterator {
            template <typename U>
            using constify_t =
                std::conditional_t<is_const, std::add_const_t<U>, U>;
...
namespace Catch {
    ...
    template <typename T> struct StringMaker<with_mocked_iterator_access<T>> {
        static std::string
...
struct MoveOnlyTestElement {
    ...
    friend bool operator==( MoveOnlyTestElement const& lhs,
                            MoveOnlyTestElement const& rhs ) {
        return lhs.num == rhs.num;
    }

    friend std::ostream& operator<<( std::ostream& out,
                                     MoveOnlyTestElement const& elem ) {
        out << elem.num;
        return out;
    }
};

TEST_CASE( "Basic use of the Contains range matcher",
           "[matchers][templated][contains]" ) {
    ...
    SECTION( "Can handle type that requires ADL-found free function begin and "
             "end" ) {
    ...
    SECTION( "Initialization with move only types" ) {
        std::array<MoveOnlyTestElement, 3> in{ { MoveOnlyTestElement{ 1 },
                                                 MoveOnlyTestElement{ 2 },
                                                 MoveOnlyTestElement{ 3 } } };
...
TEST_CASE( "Basic use of the Empty range matcher",
           "[matchers][templated][empty]" ) {
    ...
    SECTION( "Type with empty" ) { REQUIRE_THAT( has_empty{}, !IsEmpty() ); }
...
namespace {
    ...
    public:
        explicit LessThanMatcher( size_t target ): m_target( target ) {}
    ...
    LessThanMatcher Lt( size_t sz ) { return LessThanMatcher{ sz }; }
...
TEST_CASE( "Usage of the SizeIs range matcher",
           "[matchers][templated][size]" ) {
...
TEST_CASE( "Usage of AllMatch range matcher",
    ...
    SECTION( "Basic usage" ) {
        ...
        std::array<std::array<int, 5>, 5> data{ { { { 0, 1, 2, 3, 5 } },
                                                  { { 4, -3, -2, 5, 0 } },
                                                  { { 0, 0, 0, 5, 0 } },
                                                  { { 0, -5, 0, 5, 0 } },
                                                  { { 1, 0, 0, -1, 5 } } } };
...
TEST_CASE( "Usage of AnyMatch range matcher",
           "[matchers][templated][quantifiers]" ) {
...
TEST_CASE( "Usage of NoneMatch range matcher",
           "[matchers][templated][quantifiers]" ) {
...
namespace Catch {
    template <> struct StringMaker<ConvertibleToBool> {
        static std::string
...
TEST_CASE( "Usage of AllTrue range matcher",
           "[matchers][templated][quantifiers]" ) {
...
TEST_CASE( "Usage of NoneTrue range matcher",
           "[matchers][templated][quantifiers]" ) {
...
TEST_CASE( "Usage of AnyTrue range matcher",
           "[matchers][templated][quantifiers]" ) {
...
TEST_CASE( "The quantifier range matchers support types with different types "
    ...
    SECTION( "AllAnyNoneTrue" ) {
        has_different_begin_end_types<bool> diff_types{
            false, false, true, false, false };
