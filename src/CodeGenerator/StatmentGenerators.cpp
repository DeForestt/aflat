  ...
  if (!this->globalScope) {
    ...
    if (this->scope == nullptr || this->inFunction) {
      auto mod = gen::scope::ScopeManager::getInstance()->assign(
          dec->ident, dec->type, false, dec->mut);
...
void gen::CodeGenerator::genDecArr(ast::DecArr *dec, asmc::File &OutputFile) {
  ...
  links::LinkedList<gen::Symbol> *Table;
  if (this->scope == nullptr || this->inFunction) {
    gen::scope::ScopeManager::getInstance()->assign("." + dec->ident, type,
                                                    false, false);
...
void gen::CodeGenerator::genDecAssign(ast::DecAssign *decAssign,
  ...
  if (!this->globalScope) {
    if (this->scope == nullptr || this->inFunction) {
      ...
      if (dec->type.typeName != "let" &&
        ...
        expr = this->GenExpr(imply(decAssign->expr, dec->type.typeName),
                             OutputFile);
      };
      ...
      if (dec->type.typeName == "let") {
        ...
        if (expr.type.find("~") != std::string::npos) {
          ...
          if (t == nullptr)
            alert("Function Pointer type not found " + expr.type + " on line " +
                  std::to_string(decAssign->logicalLine));
          dec->type = *t;
        }
      }
    ...
    if (!this->canAssign(
            ...
            "cannot assign type {} cannot be assigned to type {}")) {
      exp =
          this->GenExpr(imply(decAssign->expr, dec->type.typeName), OutputFile);
    };
  ...
  if (dec->type.fPointerArgs.isFPointer &&
      this->typeList[dec->TypeName] == nullptr) {
    auto newType = new ast::Type();
    newType->typeName = dec->TypeName;
    newType->size = asmc::QWord;
    newType->fPointerArgs = dec->type.fPointerArgs;
...
void gen::CodeGenerator::genReturn(ast::Return *ret, asmc::File &OutputFile) {
  ...
  if (!this->canAssign(this->returnType, from.type,
...
void gen::CodeGenerator::genAssign(ast::Assign *assign,
  ...
  if (!assign->reference) {
    if (!this->canAssign(symbol->type, expr.type,
      ...
      expr =
          this->GenExpr(imply(assign->expr, symbol->type.typeName), OutputFile);
    }
  }
...
ast::Function gen::CodeGenerator::GenCall(ast::Call *call,
  ...
  if (call->modList.pos == nullptr) {
    ...
    if (func == nullptr) {
      ...
      if (smbl != nullptr) {
        if (smbl->type.typeName == "adr" ||
          ...
          if (cl == nullptr)
            alert("type is not a class " + smbl->type.typeName);
          ...
          if (f == nullptr) {
            ...
            if (parent != nullptr) {
              ...
              tname = parent->Ident;
              if (f == nullptr)
                alert("cannot preform call on type " + smbl->type.typeName +
    ...
    while (call->modList.pos != nullptr) {
      ...
      auto type = *this->typeList[last.typeName];
      ...
      if (cl != nullptr) {
        ...
        if (func == nullptr) {
          ...
          if (sym != nullptr && (sym->type.typeName == "adr" ||
            ...
            // find the type of the object
            auto objectType = *this->typeList[sym->type.typeName];
            if (objectType == nullptr)
              alert("type not found " + sym->type.typeName);
            auto objectClass = dynamic_cast<gen::Class *>(objectType);
            if (objectClass == nullptr)
              alert("type is not a class " + sym->type.typeName);
            auto f = objectClass->nameTable["_call"];
            if (f == nullptr)
              alert("cannot preform call on type " + sym->type.typeName +
                    " because it does not implement the _call "
                    "function");
            func = new ast::Function();
            func->ident = f->ident;
  ...
  while (call->Args.trail() > 0) {
    ...
    if (!exp.passable)
      this->alert("Cannot pass an lvalue of safe type " + exp.type +
                  " to a function");
    if (checkArgs) {
    ...
    if (exp.op == asmc::Float) {
      ...
      pushArg->logicalLine = call->logicalLine;

      pushArg->op = this->intArgs[argsCounter].get(asmc::QWord);

      OutputFile.text << pushArg;
...
void gen::CodeGenerator::genInc(ast::Inc *inc, asmc::File &OutputFile) {
  ...
  this->canAssign(sym->type, "int",
                  "cannot increment type {} only int can be incremented");
...
void gen::CodeGenerator::genImport(ast::Import *imp, asmc::File &OutputFile) {
  ...
  for (std::string ident : imp->imports) {
    if (this->includedClasses.contains(id + "::" + ident)) continue;
...
void gen::CodeGenerator::genContinue(ast::Continue *cont,
                                     asmc::File &OutputFile) {
