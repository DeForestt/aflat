    ...
    if (_case.pattern.veriableName.has_value()) {
      if (std::holds_alternative<ast::Type *>(alias.value)) {
        ...
        if (type->isLoan) {
          ...
          // Keep the payload's object address and declared type together.
          // Ordinary reads apply its class accessor, whose result may have a
          // completely different type (and must not be accessed a second time).
          auto mov = new asmc::Mov();
          mov->logicalLine = expr->logicalLine;
          mov->size = asmc::QWord;
