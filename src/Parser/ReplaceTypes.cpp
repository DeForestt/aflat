...
static void applyList(links::LinkedList<T *> &list,
  ...
  for (int i = 0; i < list.count; ++i) {
    T *val = list.shift();
    if (val)
      val->replaceTypes(map);
  }
  list.reset();
}
static std::string
replaceTypeString(const std::string &input,
                  const std::unordered_map<std::string, std::string> &map) {
  std::string result;
...
static void
applyTemplateTypes(std::vector<std::string> &templateTypes,
                   const std::unordered_map<std::string, std::string> &map) {
...
void Statement::replaceTypes(std::unordered_map<std::string, std::string> map) {
  ...
  if (auto buy = dynamic_cast<Buy *>(this)) {
    if (buy->expr)
      buy->expr->replaceTypes(map);
    return;
  }
  if (auto var = dynamic_cast<Var *>(this)) {
    if (map.find(var->Ident) != map.end())
      var->Ident = map[var->Ident];
  ...
  if (auto notexpr = dynamic_cast<Not *>(this)) {
    if (notexpr->expr)
      notexpr->expr->replaceTypes(map);
    return;
  }
  if (auto ifexpr = dynamic_cast<IfExpr *>(this)) {
    if (ifexpr->expr)
      ifexpr->expr->replaceTypes(map);
    if (ifexpr->trueExpr)
      ifexpr->trueExpr->replaceTypes(map);
    if (ifexpr->falseExpr)
      ifexpr->falseExpr->replaceTypes(map);
    return;
  }
  if (auto comp = dynamic_cast<Compound *>(this)) {
    if (comp->expr1)
      comp->expr1->replaceTypes(map);
    if (comp->expr2)
      comp->expr2->replaceTypes(map);
    return;
  }
  ...
  if (auto lambda = dynamic_cast<Lambda *>(this)) {
    if (lambda->function)
      lambda->function->replaceTypes(map);
    return;
  }
  ...
  if (auto declare = dynamic_cast<Declare *>(this)) {
    ...
    if (!declare->requestType.empty()) {
      ...
      if (it2 != map.end())
        declare->requestType = it2->second;
    }
    return;
  }
  ...
  if (auto decAssign = dynamic_cast<DecAssign *>(this)) {
    if (decAssign->declare)
      decAssign->declare->replaceTypes(map);
    if (decAssign->expr)
      decAssign->expr->replaceTypes(map);
    return;
  }
  if (auto decAssignArr = dynamic_cast<DecAssignArr *>(this)) {
    if (decAssignArr->declare)
      decAssignArr->declare->replaceTypes(map);
    if (decAssignArr->expr)
      decAssignArr->expr->replaceTypes(map);
    return;
  }
  ...
  if (auto destruct = dynamic_cast<Destructure *>(this)) {
    if (destruct->expr)
      destruct->expr->replaceTypes(map);
    return;
  }
  if (auto forst = dynamic_cast<For *>(this)) {
    if (forst->declare)
      forst->declare->replaceTypes(map);
    if (forst->expr)
      forst->expr->replaceTypes(map);
    if (forst->increment)
      forst->increment->replaceTypes(map);
    if (forst->Run)
      forst->Run->replaceTypes(map);
    return;
  }
  ...
  if (auto ifst = dynamic_cast<If *>(this)) {
    if (ifst->expr)
      ifst->expr->replaceTypes(map);
    if (ifst->statement)
      ifst->statement->replaceTypes(map);
    if (ifst->elseStatement)
      ifst->elseStatement->replaceTypes(map);
    if (ifst->elseIf)
      ifst->elseIf->replaceTypes(map);
    return;
  }
  if (auto ret = dynamic_cast<Return *>(this)) {
    if (ret->expr)
      ret->expr->replaceTypes(map);
    return;
  }
  if (auto seq = dynamic_cast<Sequence *>(this)) {
    if (seq->Statement1)
      seq->Statement1->replaceTypes(map);
    if (seq->Statement2)
      seq->Statement2->replaceTypes(map);
    return;
  }
  if (auto strct = dynamic_cast<Struct *>(this)) {
    if (strct->statement)
      strct->statement->replaceTypes(map);
    return;
  }
  ...
  if (auto whileSt = dynamic_cast<While *>(this)) {
    if (whileSt->expr)
      whileSt->expr->replaceTypes(map);
    if (whileSt->stmt)
      whileSt->stmt->replaceTypes(map);
    return;
  }
  if (auto cls = dynamic_cast<Class *>(this)) {
    if (cls->contract)
      cls->contract->replaceTypes(map);
    if (cls->statement)
      cls->statement->replaceTypes(map);
    return;
  }
  if (auto lambdaExpr = dynamic_cast<Lambda *>(this)) {
    if (lambdaExpr->function)
      lambdaExpr->function->replaceTypes(map);
    return;
  }
