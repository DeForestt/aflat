...
void Statement::namespaceSwap(
  ...
  if (auto expr = dynamic_cast<Expr *>(this)) {
    if (expr->extention)
      expr->extention->namespaceSwap(map);
  }
  ...
  if (auto notexpr = dynamic_cast<Not *>(this)) {
    if (notexpr->expr)
      notexpr->expr->namespaceSwap(map);
    return;
  }
  if (auto ifexpr = dynamic_cast<IfExpr *>(this)) {
    if (ifexpr->expr)
      ifexpr->expr->namespaceSwap(map);
    if (ifexpr->trueExpr)
      ifexpr->trueExpr->namespaceSwap(map);
    if (ifexpr->falseExpr)
      ifexpr->falseExpr->namespaceSwap(map);
    return;
  }
  if (auto comp = dynamic_cast<Compound *>(this)) {
    if (comp->expr1)
      comp->expr1->namespaceSwap(map);
    if (comp->expr2)
      comp->expr2->namespaceSwap(map);
    return;
  }
  ...
  if (auto lambda = dynamic_cast<Lambda *>(this)) {
    if (lambda->function)
      lambda->function->namespaceSwap(map);
    return;
  }
  ...
  if (auto decAssign = dynamic_cast<DecAssign *>(this)) {
    if (decAssign->declare)
      decAssign->declare->namespaceSwap(map);
    if (decAssign->expr)
      decAssign->expr->namespaceSwap(map);
    return;
  }
  if (auto decAssignArr = dynamic_cast<DecAssignArr *>(this)) {
    if (decAssignArr->declare)
      decAssignArr->declare->namespaceSwap(map);
    if (decAssignArr->expr)
      decAssignArr->expr->namespaceSwap(map);
    return;
  }
  if (auto call = dynamic_cast<Call *>(this)) {
    auto it = map.find(call->ident);
    if (it != map.end())
      call->imbeddedNamespace = it->second;
    applyList(call->Args, map);
    return;
  }
  if (auto destruct = dynamic_cast<Destructure *>(this)) {
    if (destruct->expr)
      destruct->expr->namespaceSwap(map);
    return;
  }
  if (auto forst = dynamic_cast<For *>(this)) {
    if (forst->declare)
      forst->declare->namespaceSwap(map);
    if (forst->expr)
      forst->expr->namespaceSwap(map);
    if (forst->increment)
      forst->increment->namespaceSwap(map);
    if (forst->Run)
      forst->Run->namespaceSwap(map);
    return;
  }
  ...
  if (auto func = dynamic_cast<Function *>(this)) {
    if (func->args)
      func->args->namespaceSwap(map);
    if (func->statement)
      func->statement->namespaceSwap(map);
    applyList(func->decoratorArgs, map);
    return;
  }
  if (auto ifst = dynamic_cast<If *>(this)) {
    if (ifst->expr)
      ifst->expr->namespaceSwap(map);
    if (ifst->statement)
      ifst->statement->namespaceSwap(map);
    if (ifst->elseStatement)
      ifst->elseStatement->namespaceSwap(map);
    if (ifst->elseIf)
      ifst->elseIf->namespaceSwap(map);
    return;
  }
  if (auto ret = dynamic_cast<Return *>(this)) {
    if (ret->expr)
      ret->expr->namespaceSwap(map);
    return;
  }
  if (auto seq = dynamic_cast<Sequence *>(this)) {
    if (seq->Statement1)
      seq->Statement1->namespaceSwap(map);
    if (seq->Statement2)
      seq->Statement2->namespaceSwap(map);
    return;
  }
  if (auto strct = dynamic_cast<Struct *>(this)) {
    if (strct->statement)
      strct->statement->namespaceSwap(map);
    return;
  }
  if (auto whileSt = dynamic_cast<While *>(this)) {
    if (whileSt->expr)
      whileSt->expr->namespaceSwap(map);
    if (whileSt->stmt)
      whileSt->stmt->namespaceSwap(map);
    return;
  }
  if (auto uni = dynamic_cast<Union *>(this)) {
    if (uni->statement)
      uni->statement->namespaceSwap(map);
  ...
  if (auto match = dynamic_cast<Match *>(this)) {
    if (match->expr)
      match->expr->namespaceSwap(map);
    for (auto &caseExpr : match->cases) {
      if (caseExpr.statement)
        caseExpr.statement->namespaceSwap(map);
    }
    return;
  }
  if (auto unionConstructor = dynamic_cast<UnionConstructor *>(this)) {
    if (unionConstructor->expr)
      unionConstructor->expr->namespaceSwap(map);
    return;
  }
  if (auto cls = dynamic_cast<Class *>(this)) {
    if (cls->contract)
      cls->contract->namespaceSwap(map);
    if (cls->statement)
      cls->statement->namespaceSwap(map);
    return;
  }
  if (auto lambdaExpr = dynamic_cast<Lambda *>(this)) {
    if (lambdaExpr->function)
      lambdaExpr->function->namespaceSwap(map);
    return;
  }
}
